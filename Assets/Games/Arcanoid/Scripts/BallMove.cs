...
namespace GameArcanoid
{
    public class BallMove : MonoBehaviour
    {
        ...
        private void Update()
        {
            if (!_isActiv) // если м€ч не активен
            {
                if (Math.Abs(_platform.transform.position.y - transform.position.y) >=
                    1.2f) // провер€ет разницу по оси Y между платформой и м€чом
                {
                    ...
                    if (_platform.transform.position.y - transform.position.y <=
                        -1.2f) // ставит его возле верхнего кра€ платформы
                    {
                        transform.position = new Vector2(transform.position.x, _platform.transform.position.y + 1.2f);
                    }

                    if (_platform.transform.position.y - transform.position.y >=
                        1.2f) // ставит его возле нижнегоы кра€ платформы
                    {
                        transform.position = new Vector2(transform.position.x, _platform.transform.position.y - 1.2f);
                    }
                }
        ...
        private void CalculationsMove() // проводим расчеты отклонени€ от платформы
        {
            ...
            float difference =
                (_ballPositionY - _platformCenterPosition); //разница между центром платформы и центром м€ча
