...
namespace GameArcanoid
{
    public class BuildBlocks : MonoBehaviour
    {
        ...
        [SerializeField]
        private List<GameObject> _blocks = new List<GameObject>(); // список блоков, которые нужно поставить

        [SerializeField]
        public static List<GameObject>
            _standartStartBlocks = new List<GameObject>(); // список стартовых блоков на сцене

        [SerializeField]
        public static List<GameObject>
            _smallStartBlocks = new List<GameObject>(); // список маленьких стартовых блоков на сцене

        [SerializeField]
        public static List<GameObject>
            _bigStartBlocks = new List<GameObject>(); // список больших стартовых блоков на сцене
        ...
        private void BuildLevel()
        {
            foreach (var item in _smallStartBlocks) //перебираем массив маленьких блоков
            {
                ...
                _block = Instantiate(_blocks[rand], _pos,
                    Quaternion.identity); // создает новый объект на месте уничтоженного
            ...
            foreach (var item in _standartStartBlocks) //перебираем массив
            {
                ...
                _block = Instantiate(_blocks[rand], _pos,
                    Quaternion.Euler(0f, 0f, -90f)); // создает новый объект на месте уничтоженного
                _block.transform.SetParent(_mainBlocks.transform);
                Debug.Log("Standart");
            }
